	...
	while (true)
	{
		...
		if ((MISC::GET_GAME_TIMER() - iVar6) > 100)
		{
			if (!PED::IS_PED_INJURED(PLAYER::PLAYER_PED_ID()))
			{
				...
				{
					if (SYSTEM::VDIST2(vVar0, ENTITY::GET_ENTITY_COORDS(Local_79[iVar3], false)) >= IntToFloat((iVar5 * iVar5)) && ENTITY::IS_ENTITY_OCCLUDED(Local_79[iVar3]))
					{
				...
				else if (ENTITY::DOES_ENTITY_EXIST(Local_79[iVar3]))
				{
					func_6(&(Local_79[iVar3]), 1, 0, 1);
				}
				if (func_3(Local_79.f_5[iVar4]))
				{
					...
					{
						if (ENTITY::IS_ENTITY_OCCLUDED(Local_79.f_5[iVar4]) || PED::IS_PED_IN_VEHICLE(PLAYER::PLAYER_PED_ID(), Local_79.f_5[iVar4], false))
						{
							func_2(&(Local_79.f_5[iVar4]));
						}
					}
				}
				else if (ENTITY::DOES_ENTITY_EXIST(Local_79.f_5[iVar4]))
				{
					func_2(&(Local_79.f_5[iVar4]));
				}
				iVar3++;
...
int func_1()
{
	...
	{
		if (ENTITY::DOES_ENTITY_EXIST(Local_79.f_5[iVar0]))
		{
			return 0;
		}
		iVar0++;
	...
	{
		if (ENTITY::DOES_ENTITY_EXIST(Local_79[iVar0]))
		{
			return 0;
		}
		iVar0++;
...
int func_5(int iParam0, var uParam1, int iParam2, int iParam3, bool bParam4, bool bParam5, float fParam6, char* sParam7, int iParam8, int iParam9, bool bParam10)
{
	if (iParam3 == 0)
	{
...
void func_19()
{
	...
	while (iVar0 <= (Local_79.f_5 - 1))
	{
		...
		{
			ENTITY::SET_ENTITY_AS_MISSION_ENTITY(Local_79.f_5[iVar0], true, true);
			vLocal_88[iVar0 /*3*/] = { ENTITY::GET_ENTITY_COORDS(Local_79.f_5[iVar0], false) };
		}
		iVar0++;
	}
	iVar0 = 0;
	while (iVar0 <= (Local_79 - 1))
	{
		...
		{
			ENTITY::SET_ENTITY_AS_MISSION_ENTITY(Local_79[iVar0], true, true);
			PED::SET_PED_RELATIONSHIP_GROUP_HASH(Local_79[iVar0], iLocal_145);
